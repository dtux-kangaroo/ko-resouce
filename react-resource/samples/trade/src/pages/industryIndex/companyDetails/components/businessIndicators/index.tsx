...
export default class BusinessIndicators extends  React.Component<IProps, IState>  {
  ...
  indexListUsingPost = () => { // 获取指标列表
    ...
    API.indexListUsingPost({
      ...
      if (success) {
        this.setState({
          dataSource: data ? data.contentList : [],
  ...
  handleMenuClick = (e: any) => {
    if( e.key == 'import' ) {
      ...
      a.href = `${URL.indexExportUsingGet}?entId=${this.props.entId}`;
      a.click();
    }
  }
  ...
  render(){
    const { dataSource, pageNo, total, editItem, selectedRowKeys, importModalVisible, modalVisible } = this.state;
    ...
    return (
      <div className="business-indicators">
        ...
        <div className="table-box">
          <Table dataSource={dataSource} scroll={{ x: 1700 }} pagination={false} columns={columns} rowSelection={rowSelection} />
